...
- [x] **T1: Ingress Component**. Done: all 4 harness checks pass (scratchpad `check.sh` +
  ...
  guard is `${var:=default}`, which doesn't help here, so the empty-`APP_HOST` risk stays a
  documented ordering step (header comment in `ingress.yaml`, and T2/T3).
  - Acceptance:
    ...
    - Ingress `${APP_NAME}-${APP_SERVICE_NAME}-${APP_ENV}` in `${APP_NAMESPACE}-${APP_ENV}`:
      `ingressClassName: traefik`, host and TLS host `${APP_HOST}`, TLS secret `…-tls`, path `/`
  ...
  - Verify (scratchpad harness, see plan):
    ...
    - Prod view renders 4 objects. Its placeholder set is exactly the 7 existing vars + `APP_HOST`
...
- [x] **T2: Enable on vm-prod + ConfigMap template keys**. Done: scratchpad `check-t2.sh` passes
  ...
  - Acceptance:
    ...
    - `k8s/configmap.example.yaml` adds `APP_HOST` and `APP_TLS_CLUSTER_ISSUER` with placeholders,
      marked "only where the `ingress` component is enabled (vm-prod); set before merging to
      `deployment`, or the Ingress matches every host".
  - Verify:
...
- [ ] **T3: Runbook (`k8s/README.md`)**
  - Acceptance:
    - New section "Expose cms-api (vm-prod)" with the prerequisites in order: DNS record, inbound
      ...
      `CORS_ORIGINS`.
    - A verify block with `curl -I http://<host>` (301/308), `curl https://<host>/health` (200),
...
- [ ] **T4: Deployment doc, techstack doc and ENTRYPOINT**
  - Acceptance:
    - `cms-api-flux-deployment.md`: "There's no Ingress" is replaced with an Ingress section
      covering the Component, the vm-prod-only mechanism, the new keys, the empty-`APP_HOST` gotcha,
