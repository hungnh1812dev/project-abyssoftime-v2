...
describe("RefreshTokenService", () => {
  ...
  it("throws UnauthorizedException when the token's subject no longer exists, and blacklists nothing", async () => {
  ...
  it("throws UnauthorizedException when the user has no role assigned, and blacklists nothing", async () => {
  ...
  it("throws UnauthorizedException when the user's assigned role no longer exists, and blacklists nothing", async () => {
  ...
  it("re-fetches the user and role fresh from the database, rotates both tokens preserving rememberMe:true, and blacklists the consumed jti as the last step", async () => {
    users.findById.mockResolvedValue(verifiedUser);
    ...
    expect(tokenBlacklistService.blacklist).toHaveBeenCalledWith({ jti: "old-jti", userId: "user-1", expiresAt: new Date(1700000000 * 1000), reason: "rotation" });
    expect(result).toEqual({ accessToken: "new-access-token", refreshToken: "new-refresh-token", refreshTokenMaxAgeMs: 30 * 24 * 60 * 60 * 1000 });
  });

  it("rotates both tokens preserving rememberMe:false, and blacklists the consumed jti", async () => {
    users.findById.mockResolvedValue(verifiedUser);
    roles.findById.mockResolvedValue(adminRole);
    jwtTokenService.signAccessToken.mockReturnValue("new-access-token");
    jwtTokenService.signRefreshToken.mockReturnValue("new-refresh-token");
  ...
  it("skips blacklisting when the consumed token carries no jti (pre-migration token), and still rotates successfully", async () => {
    users.findById.mockResolvedValue(verifiedUser);
    roles.findById.mockResolvedValue(adminRole);
    jwtTokenService.signAccessToken.mockReturnValue("new-access-token");
    jwtTokenService.signRefreshToken.mockReturnValue("new-refresh-token");
  ...
  it("skips blacklisting when jti is present but exp is missing", async () => {
    users.findById.mockResolvedValue(verifiedUser);
    roles.findById.mockResolvedValue(adminRole);
    jwtTokenService.signAccessToken.mockReturnValue("new-access-token");
    jwtTokenService.signRefreshToken.mockReturnValue("new-refresh-token");
