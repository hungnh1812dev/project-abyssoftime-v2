...
- **As a public-site or cms-admin user**, I call `https://<APP_HOST>/api/v1/...`,
...
| --- | --- | --- |
| `APP_HOST` | Public hostname of cms-api | `cms-api.example.com` |
...
1. A DNS `A`/`AAAA` record for `APP_HOST` pointing at the VPS. Inbound 80 and 443 are open.
...
kubectl kustomize <scratchpad>/prod-view | APP_NAME=a APP_SERVICE_NAME=s APP_NAMESPACE=n APP_ENV=prod \
  APP_PORT=3000 APP_IMAGE_REPO=r APP_IMAGE_TAG=1-abcdef0-amd64 APP_HOST=api.test \
...
apps/cms-api/k8s/configmap.example.yaml           # + APP_HOST, APP_TLS_CLUSTER_ISSUER
...
2. The prod view renders a Deployment, Service, Ingress and Middleware. Its placeholder set is
   exactly the 7 existing vars + `APP_HOST` + `APP_TLS_CLUSTER_ISSUER`.
...
6. Manual, by the owner after deploy: `curl -I http://<host>` returns 301/308 to https,
   `curl https://<host>/health` returns 200 with a valid Let's Encrypt cert, and
...
- [ ] `configmap.example.yaml` documents `APP_HOST` and `APP_TLS_CLUSTER_ISSUER` as prod/Ingress-only.
