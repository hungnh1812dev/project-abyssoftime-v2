...
export const CvNewExperience = ({ experiences, commonText }: CvNewExperienceProps) => {
  return (
    <CvNewSection title={commonText.text["work-experience"] ?? "Work Experience & Key Projects"} id="experience">
        ...
        {experiences.map((group, groupIdx) => (
          <div key={groupIdx}>
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 rounded-r-md border-l-4 border-primary bg-muted px-3 py-2">
              <h4 className="text-sm font-bold text-foreground">{group.company}</h4>
              <span className="text-xs text-foreground/55">{[group.location, group.period].filter(Boolean).join(" · ")}</span>
            </div>
                  ...
                  {role.projects && role.projects.length > 0 && (
                    <div className="mt-3 space-y-3">
                      {role.projects.map((project, projectIdx) => (
                        <div key={projectIdx} className="rounded-r-md border-l-[3px] border-primary/50 bg-muted/60 p-3 print:break-inside-avoid">
                          <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-0.5">
                            ...
                            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
                              {project.liveLink && (
                                <span className="text-xs text-foreground/60">
                                  <span className="font-semibold text-foreground/50">{commonText.text["live"] ?? "Live"}:</span> {project.liveLink}
                                </span>
                              )}
                              {project.responsitoryLink && (
                                <span className="text-xs text-foreground/60">
                                  <span className="font-semibold text-foreground/50">{commonText.text["github"] ?? "GitHub"}:</span> {project.responsitoryLink}
                                </span>
                              )}
                            </div>
                          )}
