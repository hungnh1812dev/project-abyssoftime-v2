...
const isActive = (currentPath: string, item: HeaderNavItem): boolean => {
  const matches = (href: string) => (href === "/" ? currentPath === "" || currentPath === "/" : currentPath === href);
  return matches(item.link) || item.subNavigations.some((sub) => matches(sub.link));
};
    ...
    <nav className="hidden items-center gap-1 md:flex">
      {nav.map((item) =>
        item.subNavigations.length > 0 ? (
          <DropdownMenu key={item.link}>
            <DropdownMenuTrigger
              className={cn(
                "flex items-center gap-1 rounded-md px-3 py-2 text-sm outline-none transition-colors",
                isActive(currentPath, item) ? "font-medium text-foreground" : "text-muted-foreground hover:text-foreground",
              )}>
              {item.title}
